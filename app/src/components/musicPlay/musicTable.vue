<template>
    <div class="musicTable">
        <div :class="{ selectedTable: isPlayList, noselectedTable: !isPlayList }" @click="changeNowPlay">播放列表</div>
        <div :class="{ selectedTabled: isHisPlayList, noselectedTabled: !isHisPlayList }" @click="changeHisPlay">历史记录
        </div>
        <div style="width: 100%; height: 5%; position: relative">
            <span style="font-size: 6px; opacity: 0.5; margin-left: -80%;">总{{ playList.length }}首</span>
            <div class="clearAll" @click="cleanPlayList">
                <i class="iconfont icon-lajitong"></i>清空
            </div>
            <!-- 播放列表 -->
            <div v-show="isPlayList">
                <!-- 无歌曲 -->
                <div v-show="!playList[0]" style="margin-top: 50%; margin-left: 50%; transform: translate(-40%, -50%)">
                    请添加歌曲</div>
                <!-- 有歌曲 -->
                <div :class="{ songMesSin: index % 2 === 0, songMesDou: index % 2 !== 0 }"
                    v-for="(item, index) in playList" :key="index" background="#f9f9f9" @dblclick="startSong(item)">
                    <div style="
            font-size: 10px;
            position: absolute;
            top: 20px;
          ...
          " v-show="item.id === songId && isPlaying">
                    ...
                    <i class="iconfont icon-zanting"
                        ...
                        v-show="item.id === songId && !isPlaying"></i>
                    <div :class="{
                    ...
                    <div :class="{ startSongAurtor: item.id === songId }" style="
            position: absolute;
            width: 120px;
            left: 45%;
            white-space: nowrap;
          ...
          ">
                        <span style="cursor: pointer" v-for="(item, index) in item.ar" :key="index">{{ index === 0 ?
                        item.name : "/" + item.name }}</span>
                    </div>
                    <span style="position: absolute; left: 80%">{{
                        Math.floor(item.dt / 1000) | timeFormat
                    }}</span>
                    <el-popconfirm title="确定要将该歌曲从列表中删除吗？" @confirm="delPlayListSong(item.id)">
                        <i class="el-icon-close" slot="reference" style="
              position: absolute;
              top: 35%;
              right: 3%;
              font-size: 15px;
            ...
            <div v-show="isHisPlayList">
                <div v-show="!hisMusicList[0]"
                    style="margin-top: 50%; margin-left: 50%; transform: translate(-40%, -50%)">
                    请添加歌曲
                </div>
                <!-- 历史列表 -->
                <div :class="{ songMesSin: index % 2 === 0, songMesDou: index % 2 !== 0 }"
                    v-for="(item, index) in hisMusicList" :key="index" background="#f9f9f9" @dblclick="startSong(item)">
                    <div style="
            font-size: 10px;
            position: absolute;
            top: 20px;
          ...
          ">
                    ...
                    <el-popconfirm title="确定要将该歌曲从列表中删除吗？" @confirm="delHisListSong(item.id)">
                        <i class="el-icon-close" slot="reference" style="
              position: absolute;
              top: 35%;
              right: 3%;
